...
namespace Tests\Feature\Shared\Cart;
...
class CartTest extends TestCase
{
    ...
    public function test_updateItem_productid_changed()
    {
        $response = $this->call('GET', '/order-added', ['productId'=>17, 'quantity'=>1, 'subItems'=>'']);
        $response->assertStatus(200);
        $cart = new Cart(Session::get('cart'));
        $this->assertEquals('Small Side', $cart->items[1]['productItem']->name);

        $response = $this->call('GET', '/order-updated', ['serialNumber'=>1, 'productId'=>18, 'quantity'=>1, 'subItems'=>'']);
        $response->assertStatus(200);
        $cart = new Cart(Session::get('cart'));
        $this->assertEquals('Regular Side', $cart->items[2]['productItem']->name);
    }

    public function test_updateItem_kids_meal_fountain_soft_drink_changed_to_bottle_water_extra_charge()
    {
        $subItems = array();
        ...
        $drinkItem = array('category'=>'Drink', 'id'=>1, 'quantity'=>1, 'selectBoxId'=>1);
        array_push($subItems, $drinkItem);

        $response = $this->call('GET', '/order-added', ['productId'=>16, 'quantity'=>1, 'subItems'=>json_encode($subItems)]);
        $response->assertStatus(200);
        $cart = new Cart(Session::get('cart'));
        $this->assertEquals(4.99, $cart->totalPrice);
        ...
        array_push($subItems, $drinkItem);
        $response = $this->call('GET', '/order-updated', ['serialNumber'=>1, 'productId'=>16, 'quantity'=>1, 'subItems'=>json_encode($subItems)]);
        $response->assertStatus(200);
        $cart = new Cart(Session::get('cart'));
        $this->assertEquals(5.74, $cart->totalPrice);
    }

    public function test_canNewItemBeMerged_yes()
    {
        $subItems = array();
        $sideItem = array('category'=>'Side', 'id'=>1, 'quantity'=>1);
        array_push($subItems, $sideItem);
        ...
        $response = $this->call('GET', '/order-added', ['productId'=>16, 'quantity'=>2, 'subItems'=>json_encode($subItems)]);
        $response->assertStatus(200);
        $cart = new Cart(Session::get('cart'));
        $this->assertEquals(14.97, $cart->totalPrice);
        $this->assertEquals(1, count($cart->items));
    }

    public function test_canNewItemBeMerged_no()
    {
        $subItems = array();
        $sideItem = array('category'=>'Side', 'id'=>1, 'quantity'=>1);
        array_push($subItems, $sideItem);
